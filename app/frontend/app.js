...
function inspectArchComponent(id) {
  const details = {
    ...
    sca: {
      ...
      tech: 'Trivy FS Scan',
      role: 'Checks node dependencies manifests (package.json) against matching global CVE vulnerability lists.',
      failure: 'Malicious supply chain dependency updates containing prototype pollutions load at runtime.'
    },
    secrets: {
      name: 'Gitleaks Secrets Auditing',
      tech: 'Gitleaks CLI',
      role: 'Scans commit configurations checking for plain-text AWS/database credentials and keys leaks.',
      failure: 'API access keys committed in plaintext files get automatically indexed by scraper bots in seconds.'
    },
    sbom: {
      name: 'Syft Software Bill of Materials',
      tech: 'Syft Container cataloger',
      role: 'Generates structured packages bill list of packages (CycloneDX format) providing licensing transparency.',
      failure: 'Compliance audits fail due to lack of software tracking inventory.'
    },
    cosign: {
      name: 'Cosign Cryptographic Signature Verification',
      tech: 'Cosign CMK Signatures',
      role: 'Signs built image digests with developer private keys to prove container origin authenticity.',
      failure: 'Unsigned registry assets get intercepted by man-in-the-middle exploits and deploy bad packages.'
    },
    kyverno: {
      name: 'Kyverno Policy Admission Webhook',
      tech: 'Kyverno Kubernetes Controllers',
      role: 'Validates pods specs on admission. Rejects containers executing with UID 0 (Root) or missing memory limits.',
      failure: 'Pods escape isolation boundaries because security context drop permissions were not enforced.'
    }
  };
...
function switchDocTab(docId) {
  ...
  // Simple markdown-style line break parser
